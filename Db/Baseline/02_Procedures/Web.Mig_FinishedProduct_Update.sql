IF OBJECT_ID ('[Web].[Mig_FinishedProduct_Update]') IS NOT NULL
	DROP PROCEDURE [Web].[Mig_FinishedProduct_Update]	
GO


Create PROCEDURE [Web].[Mig_FinishedProduct_Update]
	@Id INT 
AS 
BEGIN 
UPDATE RUG_Design 
...
UPDATE RUG_CarpetSku
SET RUG_CarpetSku.Design = Ig.Code
FROM Web.Products H 
LEFT JOIN Web.ProductGroups Pg ON H.ProductGroupId = Pg.ProductGroupId 
LEFT JOIN dbo.ItemGroup Ig On  'Product Group-' + Convert(Varchar,H.ProductGroupId) = Ig.OMSId 
WHERE H.ProductId = @Id
AND RUG_CarpetSku.OMSId = 'Produdct-' + Convert(Varchar,H.ProductId)
