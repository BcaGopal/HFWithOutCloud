...
namespace Jobs.Controllers
{
    ...
    public class UnitController : System.Web.Mvc.Controller
    {
        ...
        {
            var DocType = new DocumentTypeService(_unitOfWork).FindByName(MasterDocTypeConstants.Unit);
            int DocTypeId = 0;

            if (DocType != null)
                DocTypeId = DocType.DocumentTypeId;
            else
                return View("~/Views/Shared/InValidSettings.cshtml").Warning("Document Type named " + MasterDocTypeConstants.Unit + " is not defined in database.");
