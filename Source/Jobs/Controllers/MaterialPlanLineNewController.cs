...
namespace Jobs.Controllers
{
    ...
    public class MaterialPlanLineNewController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult DeletePost(MaterialPlanLineViewModel vm)
        {
            ...
            {
                var JobOrderLineCreated = (from L in db.JobOrderLine where L.ProdOrderLineId == item.ProdOrderLineId select L).FirstOrDefault();
                if (JobOrderLineCreated != null)
                {
                    string message = "Job Order is already created for this plan,Can't delete.";
                    ModelState.AddModelError("", message);
                    return PartialView("_Create", vm);
                }

                var MaterialPlanForProdOrdersForProdOrderLine = (from L in db.MaterialPlanForProdOrder where L.ProdOrderLineId == item.ProdOrderLineId select L).FirstOrDefault();
                if (MaterialPlanForProdOrdersForProdOrderLine != null)
                {
                    string message = "Material Plan is already created for this plan,Can't delete.";
                    ModelState.AddModelError("", message);
                    return PartialView("_Create", vm);
                }
