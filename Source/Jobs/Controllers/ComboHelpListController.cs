    ...
    public class ComboHelpListController : System.Web.Mvc.Controller
    {
        ...
        public JsonResult SetSingleProdOrder(int Ids)
        {
            ...
            IEnumerable<ProdOrderHeader> prod = from p in db.ProdOrderHeader
                                            where p.ProdOrderHeaderId == Ids
                                            select p;

            ProductJson.id = prod.FirstOrDefault().ProdOrderHeaderId.ToString();
            ProductJson.text = prod.FirstOrDefault().DocNo;

            return Json(ProductJson);
        }

        public JsonResult GetPlanNos(string searchTerm, int pageSize, int pageNum)
        {
            var Query = cbl.GetPlanNos(searchTerm);
            var temp = Query.Skip(pageSize * (pageNum - 1)).Take(pageSize).ToList();
        ...
        public JsonResult SetSinglePlanNo(string Ids)
        {
            ComboBoxResult PlanNoJson = new ComboBoxResult();

            PlanNoJson.id = Ids;
            PlanNoJson.text = Ids;

            return Json(PlanNoJson);
        }
        public JsonResult SetPlanNo(string Ids)
        {
            string[] subStr = Ids.Split(',');
            List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
            for (int i = 0; i < subStr.Length; i++)
