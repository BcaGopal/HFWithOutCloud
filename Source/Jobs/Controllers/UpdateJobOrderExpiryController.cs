...
{
    [Authorize]
    public class UpdateJobOrderExpiryController : System.Web.Mvc.Controller
    {

        private ApplicationDbContext db = new ApplicationDbContext();

        IUpdateJobExpiryService _JobExpiryService;
        ...
        public ActionResult DocumentTypeIndex(int id)//DocumentCategoryId
        {
            ...
            {
                if (p.Count == 1)
                    return RedirectToAction("UpdateJobExpiry", new { id = p.FirstOrDefault().DocumentTypeId });
            }

            return View("DocumentTypeList", p);
        }

        public ActionResult UpdateJobExpiry(int id)
        {
            ViewBag.DocTypeName = new DocumentTypeService(_unitOfWork).Find(id).DocumentTypeName;
            ViewBag.Id = id;
            return View("UpdateJobExpiry");
        }
        ...
        public class DataTableData
        {
            ...
            public List<JobOrderForExpiryViewModel> data { get; set; }
        }
        ...
        private List<JobOrderForExpiryViewModel> FilterData(ref int recordFiltered, ref int recordTotal, int start, int length, string search, int sortColumn, string sortDirection, int DueDays,bool ShowExpired,int DocType)
        {
            ...
            IQueryable<JobOrderHeader> _data = from p in db.JobOrderHeader
            ...
            List<JobOrderForExpiryViewModel> list = new List<JobOrderForExpiryViewModel>();
            ...
            list = _data.Select(m => new JobOrderForExpiryViewModel { DocDate = m.DocDate, DocTypeName = m.DocType.DocumentTypeName, DueDate = m.DueDate, Reason = m.Remark, Revised = (m.DueDate != m.ActualDueDate ? "Yes" : "No"), JobOrderHeaderId = m.JobOrderHeaderId, JobOrderNo = m.DocNo })
                .Skip(start).Take(length).ToList();

            return list.Select(m => new JobOrderForExpiryViewModel { SDocDate = m.DocDate.ToString("dd/MMM/yyyy"), DocTypeName = m.DocTypeName, SDueDate = m.DueDate.Value.ToString("dd/MMM/yyyy"), Reason = m.Reason, Revised = m.Revised, JobOrderHeaderId = m.JobOrderHeaderId, JobOrderNo = m.JobOrderNo }).ToList();

        }

        //public JsonResult PendingJobOrdersToReview(int id)//DocTypeId
        //{
        //    var temp = _JobExpiryService.GetExcessJobOrders(id).ToList();
        ...
        public ActionResult UpdateDueDate(int HeaderId, DateTime DueDate, string Reason)
        {
            ...
            bool Flag = _JobExpiryService.UpdateJobOrderExpiry(HeaderId, Reason, User.Identity.Name, DueDate);
        ...
        //{
        //    bool Flag = _JobExpiryService.DisApproveExcessStock(LineId, User.Identity.Name);
