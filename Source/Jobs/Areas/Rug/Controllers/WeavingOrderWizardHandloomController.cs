...
{
    [Authorize]
    public class WeavingOrderWizardHandloomController : System.Web.Mvc.Controller
    {
        ...
        public WeavingOrderWizardHandloomController(IJobOrderHeaderService PurchaseOrderHeaderService, IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
        ...
        public ActionResult ConfirmProdOrderList(List<WeavingOrderWizardViewModel> Selected)
        {
            ...
            if (!BarCodesBased)
                return Json(new { Success = "URL", Data = "/Rug/WeavingOrderWizardHandloom/Create" }, JsonRequestBehavior.AllowGet);
            else
                return Json(new { Success = "URL", Data = "/Rug/WeavingOrderWizardHandloom/FirstBarCode" }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult FirstBarCode()
        {
            ...
            var DealUnitName = new UnitService(_unitOfWork).Find(UnitConstants.SqYard);

            var OtherDealUnitName = string.IsNullOrEmpty(settings.DealUnitId) ? DealUnitName : new UnitService(_unitOfWork).Find(settings.DealUnitId);
            ...
            foreach (var item in ProdOrders)
            {
                ...
                item.DealUnitId = UnitConstants.SqYard;
                item.DealUnitName = DealUnitName.UnitName;
                //item.UnitConversionMultiplier = new JobOrderLineService(_unitOfWork).GetUnitConversionForProdOrderLine(item.ProdOrderLineId, settings.UnitConversionForId, item.DealUnitId);                
                item.OtherDealUnitId = string.IsNullOrEmpty(settings.DealUnitId) ? UnitConstants.SqYard : settings.DealUnitId;
                item.OtherDealUnitName = OtherDealUnitName.UnitName;
                item.OtherAmount = item.UnitConversionMultiplier * item.Qty * item.Rate;
            }
        ...
        public ActionResult FirstBarCodePost(WeavingOrderWizardMasterDetailModel vm)
        {
            ...
            foreach (var item in vm.WeavingOrderWizardViewModel)
            {
                ...
                Wizard.DealUnitId = item.DealUnitId;
                Wizard.DealUnitName = item.DealUnitName;
                Wizard.OtherAmount = item.OtherAmount;
                Wizard.OtherDealUnitId = item.OtherDealUnitId;
                Wizard.OtherDealUnitName = item.OtherDealUnitName;

                Wizard.UnitConversionMultiplier = item.UnitConversionMultiplier;
                if (item.RefDocLineId.HasValue)
                {
            ...
            if (ModelState.IsValid && (TimePlanValidation || Continue))
            {
                ...
                if (svm.JobOrderHeaderId <= 0)
                {
                    ...
                    if (ProdOrdersAndQtys.Count() > 0)
                    {
                        ...
                        s.UnitConversionForId = Settings.UnitConversionForId;
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            foreach (var SelectedProdOrd in ProdOrdersAndQtys.Where(m => m.Qty > 0))
                            {
                                if (SelectedProdOrd.ProdOrderLineId > 0 && !SelectedProdOrd.RefDocLineId.HasValue)
                                {
                                    ...
                                    if (SelectedProdOrd.Qty <= bal.BalQty)
                                    {
                                        ...
                                        line.Amount = Math.Round(SelectedProdOrd.UnitConversionMultiplier * SelectedProdOrd.Rate * SelectedProdOrd
                                            .Qty, 2);
                                        line.Rate = line.Amount / line.DealQty;
                                        line.Sr = Sr++;
                                        ...
                                        JobOrderLineExtended LineEx = new JobOrderLineExtended();
                                        LineEx.JobOrderLineId = line.JobOrderLineId;
                                        LineEx.OtherUnitId = SelectedProdOrd.OtherDealUnitId;
                                        LineEx.OtherUnitConversionMultiplier = SelectedProdOrd.UnitConversionMultiplier;
                                        LineEx.OtherQty = SelectedProdOrd.OtherQty * LineEx.OtherUnitConversionMultiplier;
                                        LineEx.OtherRate = SelectedProdOrd.Rate;
                                        //LineEx.OtherAmount = LineEx.OtherUnitConversionMultiplier * LineEx.OtherRate * LineEx
                                        //    .OtherQty;
                                        LineEx.OtherAmount = LineEx.OtherRate * LineEx
                                            .OtherQty;

                                        LineEx.ObjectState = Model.ObjectState.Added;

                                        new JobOrderLineExtendedService(_unitOfWork).Create(LineEx);
                                ...
                                else if (SelectedProdOrd.ProdOrderLineId > 0 && SelectedProdOrd.RefDocLineId.HasValue)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(SelectedProdOrd.SelectedBarCodes) && SelectedProdOrd.SelectedBarCodes.Split(',').Count() <= bal.BalQty)
                                    {
                                        ...
                                        foreach (var BarCode in SelectedProdOrd.SelectedBarCodes.Split(',').Select(Int32.Parse).ToList())
                                        {
                                            ...
                                            LineEx.OtherUnitId = SelectedProdOrd.OtherDealUnitId;
                                            LineEx.OtherUnitConversionMultiplier = SelectedProdOrd.UnitConversionMultiplier;
                                            LineEx.OtherQty = (SelectedProdOrd.OtherQty / SelectedProdOrd.Qty);
                                            LineEx.OtherRate = SelectedProdOrd.Rate;
                                            //LineEx.OtherAmount = LineEx.OtherUnitConversionMultiplier * LineEx.OtherRate * LineEx
                                            //    .OtherQty;
                                            LineEx.OtherAmount = Math.Round(SelectedProdOrd.UnitConversionMultiplier * (LineEx.OtherRate * LineEx
                                            .OtherQty), 2);
                                            ...
                                            //line.Amount = Math.Round(SelectedProdOrd.UnitConversionMultiplier * SelectedProdOrd.Rate * (SelectedProdOrd.OtherQty / SelectedProdOrd.SelectedBarCodes.Split(',').Count()), 2);
                                            line.Amount = LineEx.OtherAmount;
                                            line.Rate = line.Amount / line.DealQty;
                                            line.Sr = Sr++;
                                            line.CreatedDate = DateTime.Now;
                                            line.ModifiedDate = DateTime.Now;
