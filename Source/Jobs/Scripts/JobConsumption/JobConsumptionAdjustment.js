  ...
  function ($scope, $window, $log, $http, uiGridConstants, uiGridExporterConstants, uiGridExporterService, uiGridTreeViewConstants, $timeout) {
      ...
      $scope.gridOptions = {
          ...
          onRegisterApi: function (gridApi) {
              ...
              gridApi.edit.on.afterCellEdit($scope, function (rowEntity, colDef, newValue, oldValue) {
                  var Bal = parseFloat(rowEntity.BalQty);
                  var CnQty = parseFloat(rowEntity.ConsumeQty);
                  if (Bal < CnQty) {
                      rowEntity.ConsumeQty = rowEntity.BalQty;
                  }
          ...
          $.ajax({
              ...
              success: function (result) {
                  ...
                  if (result.Success == true) {
                      ...
                      $scope.gridOptions.columnDefs.push({ field: 'LotNo', width: 70, visible: true, cellClass: 'cell-text', headerCellClass: 'header-text', enableCellEdit: false });
                      $scope.gridOptions.columnDefs.push({ field: 'PlanNo', width: 70, visible: true, cellClass: 'cell-text', headerCellClass: 'header-text', enableCellEdit: false });
                      ...
                      $scope.gridOptions.columnDefs.push({ field: 'LineRemark', name: 'Remark', width: 70, visible: true, cellClass: 'cell-text-c', headerCellClass: 'header-text', enableCellEdit: true, enableCellEditOnFocus: true });
      ...
      $scope.SaveRecord = function () {
          ...
          $scope.gridApi.core.getVisibleRows($scope.gridApi.grid).some(function (rowItem) {
              ...
              PostedViewModel[i].JobReceiveHeaderId = $('#JobReceiveHeaderId').val();
