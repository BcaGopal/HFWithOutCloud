IF object_id ('[Web].[spRep_ProdOrderStatusDetail]') IS NOT NULL 
 DROP Procedure  [Web].[spRep_ProdOrderStatusDetail]
GO 


CREATE PROCEDURE [Web].[spRep_ProdOrderStatusDetail]
...
SET @ReportName =
(
CASE WHEN @Dimension1IdCnt > 0 AND @Dimension2IdCnt > 0 THEN 'ProdOrderStatusDetailWithDoubleDimension'
	 WHEN @Dimension1IdCnt > 0 AND @Dimension2IdCnt = 0 THEN 'ProdOrderStatusDetailWithSingleDimension'
	 WHEN @Dimension1IdCnt = 0 AND @Dimension2IdCnt > 0 THEN 'ProdOrderStatusDetailWithSingleDimension'
	 WHEN @Dimension1IdCnt = 0 AND @Dimension2IdCnt = 0 THEN 'ProdOrderStatusDetail'
END 
)
...
P.ProductName, U.UnitName, H.Specification, 
Pt.ProductTypeName,
...
H.ProdOrderLineId, VGoodsReceipt.ReceiveNo,VGoodsReceipt.ReceiveDate,
H.OrderQty, H.AmendmentQty, H.CancelQty, IsNull(VGoodsReceipt.Qty,0) AS ReceiveQty, 
H.OrderQty + H.AmendmentQty - H.CancelQty - IsNull(VJobRecSummary.Qty,0)  AS BalanceQty,
...
@ReportName + '.rdl' AS ReportName, 'Production Order Status Detail' AS ReportTitle, NULL AS SubReportProcList
FROM Web.FProdOrder(@StatusOnDate, @Site,@Division, @FromDate, @ToDate, @DocumentType, @Process,@Product, @ProdOrderHeaderId) AS H 
LEFT JOIN 
(
...
) AS VJobRecSummary ON H.ProdOrderLineId = VJobRecSummary.ProdOrderLineId
LEFT JOIN 
(
	SELECT L.ProdOrderLineId, L.Qty AS Qty,  
	L.DocDate AS ReceiveDate, L.DocNo AS ReceiveNo
	FROM Web.FJobOrder(@StatusOnDate, @Site,@Division, @FromDate, @ToDate, NULL , NULL ,@Process, @Product, NULL ) L	
