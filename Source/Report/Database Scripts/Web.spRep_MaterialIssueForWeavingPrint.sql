...
/****** Object:  StoredProcedure [Web].[spRep_MaterialIssueForWeavingPrint]    Script Date: 07/Sep/2015 5:37:49 PM ******/
IF object_id ('[Web].[spRep_MaterialIssueForWeavingPrint]') IS NOT NULL 
DROP PROCEDURE [Web].[spRep_MaterialIssueForWeavingPrint]
/****** Object:  StoredProcedure [Web].[spRep_MaterialIssueForWeavingPrint]    Script Date: 07/Sep/2015 5:37:49 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE Procedure  [Web].[spRep_MaterialIssueForWeavingPrint](@Id INT)
As
BEGIN
SELECT 
...
P.Mobile AS MobileNo,D1.Dimension1Name AS shade,
 ...
 PD.ProductName AS  StockName,L.Qty,U.UnitName, isnull(U.DecimalPlaces,0) AS DecimalPlaces,
  (CASE WHEN  RLP.ProductName=PD.ProductName THEN NULL ELSE RLP.ProductName END) AS ItemName,
 CC.CostCenterName AS CostCenterName,CCD.Design AS Design,DT.DocumentTypeName AS ReportTitle,H.GatePassHeaderId,
 'ProcGatePassPrint1 ' + convert(NVARCHAR,ISNULL(H.GatePassHeaderId,0)) +', ' + '''' +  DT.DocumentTypeShortName +'-'+ H.DocNo +''''   AS SubReportProcList,'MaterialIssueForWeaving.rdl'  as  ReportName 
  FROM 
(
